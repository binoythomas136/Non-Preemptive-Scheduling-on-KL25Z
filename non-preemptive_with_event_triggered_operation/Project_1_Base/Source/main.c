	...
	if (PTE->PDIR & MASK(CONFIG1_POS)){       // Checking for PTE3 Bit, If Floating run DEMO Code
		if (!init_mma()) {											/* init mma peripheral */
			...
			RTCS_Add_Task(Task_Motion_Sensor, 0, TICKS(TASK_MOTION_SENSOR_FREQ_HZ)); // Run periodically
				...
				RTCS_Add_Task(Task_Motion_Sensor_FSM, 0, TICKS(TASK_MOTION_SENSOR_FREQ_HZ)); // Task Moton Sensor with Higher Priority and 0 Period for Event trigerring
				RTCS_Add_Task(Task_I2C_Server, 1, TICKS(TASK_MOTION_SENSOR_FREQ_HZ));        // I2C Task with Lower Priority and 0 Period for Event trigerring
	}
	RTCS_Run_Scheduler();
